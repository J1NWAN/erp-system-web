...
| `/weekly?tab=view\|write&week=YYYY-MM-DD` | 주간업무보고 조회 / 작성 (week 는 그 주의 아무 날짜) |
| `/leave?tab=new\|my\|cal&month=YYYY-MM&dept=` | 휴가 신청 / 내역 / 팀 캘린더 |
| `/approve` | 결재함 |
...
| `test:calendar` | 주 이동(지난 주/앞으로 올 주 규칙, ISO 주차, 탭 전환 시 유지), 달 이동(요일 정렬, 4~6주 그리드, 연도 경계, 오늘 강조), 부서 필터 |
...
시안이 "오늘"로 삼는 날짜는 `data.TODAY` = **2026-08-05** 하나뿐이다.
헤더 날짜칩, 주차 계산, 캘린더의 오늘 강조가 모두 여기서 나온다.

**주간업무보고** — `‹ ›` 는 `?week=` 를 바꾸는 링크다. 값은 그 주의 아무 날짜여도
되고 서버가 월요일로 맞춘다. 주차 번호는 ISO 기준이라 연말이 다음 해 1주차가
되기도 한다. 시드 데이터가 있는 32주차(8/3–8/7)는 원본 시안 그대로 보여주고,
다른 주차는 규칙으로 만든다 — 지난 주는 전원 승인, 아직 오지 않은 주는 전원 미제출.
요약 카드의 건수·완료율도 그 주의 제출 인원에서 계산한다.

**팀 휴가 캘린더** — `‹ ›` 는 `?month=` 를 바꾼다. 그리드는 일요일 시작이고 행 수는
그 달에 맞춰 4~6주로 달라진다.

> 원본 시안의 캘린더는 35칸으로 고정돼 있어 **8월 30·31일이 잘려 나갔다.**
> 실제 달력으로 바꾸면서 함께 고쳤다. 8월은 6주 그리드가 된다.

부서 선택은 `?dept=` 를 바꿔 다시 불러온다. 기본값 개발1팀에서 보이는 8월 일정은
시안과 같고, "전체 부서"로 바꾸면 개발2팀 · 경영지원팀 일정이 더해진다.
...
- **날짜 입력이 카드를 넘지 않는다.** iOS Safari 의 `input[type=date]` 는 자체 기본
  스타일에서 나오는 고유 폭이 커서, 그리드 칸(`min-width:auto`)이 그 폭만큼 벌어지며
