/* ==========================================================================
   ERP — 휴가 화면
   신청 폼(종류·구분 칩, 사용 일수 계산) · 팀 캘린더 부서 필터
   ========================================================================== */

/* --- 팀 캘린더 부서 필터 ---------------------------------------------------
   달 이동은 링크로 처리하지만 부서는 select 라 JS 로 같은 주소를 다시 연다. */

(function () {
  "use strict";

  const pick = document.querySelector("[data-cal-dept]");
  if (!pick) return;

  pick.addEventListener("change", () => {
    window.location.href = pick.dataset.calUrl + encodeURIComponent(pick.value);
  });
})();
